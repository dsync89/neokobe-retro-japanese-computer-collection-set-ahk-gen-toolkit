...
is_add_last_zip_only = False # only add the last zip found per game title
...
def filter_game_titles(game_titles, filter, is_add_last_zip_only, list_exclude):
    zip_files = []
...
def populate_game_titles(root_folder, game_titles, list_exclude):
    ...
    for root, dirs, files in os.walk(root_folder):
        ...
        for file in files:
            if file.endswith('.zip'):
                ...
                # skip if the game is of type [Doujin]
                # pattern = r"\[Compilation\]|\[BIOS\]|\[Utility\]|\[OS\]|\[Miscellaneous\]|\[Doujin & Homebrew\]|\[extras\]"
                ...
                for pattern in patterns:
                    matches = re.findall(pattern, zip_path)
                    if matches:
                        print(f"Pattern: {pattern}")
                        print(f"Matches: {matches}")
                        print("Skipping zip: {} because it matches pattern".format(zip_path))
                        has_match = True
                        continue             
...
if __name__ == "__main__":
    ...
    populate_game_titles(ROM_DIR, game_titles, list_exclude)
    ...
    print()
    user_input = input("Please enter a a media type to process [fd, hd, cd]: ")
    ...
    zip_files = filter_game_titles(game_titles, media_type, is_add_last_zip_only, list_exclude)
    print_list("Game List: ", zip_files)    
    print("Found %d zip files to extract" % len(zip_files))

    print_list("Retool 1G1R Exclude List: ", list_exclude)

    # rename each row in zip files to just basename and sort it alphabetically
    zip_files = [os.path.splitext(os.path.basename(path))[0] for path in zip_files]
    zip_files = sorted(zip_files)

    with open("include-{}.txt".format(media_type), 'w') as file:
        for row in zip_files:
            # each row is the absolute zip path, we just want the base name
            # row = os.path.splitext(os.path.basename(row))[0]
            file.write(row + "\n")

    print("Open exclude.txt and copy all contents to Retool 1G1R Exclude Options")
